...
// .wrangler/tmp/bundle-HvQAwE/checked-fetch.js
...
// .wrangler/tmp/bundle-HvQAwE/strip-cf-connecting-ip-header.js
...
async function checkAccountLockout(db, userId) {
  ...
  const now2 = Math.floor(Date.now() / 1e3);
  if (user.locked_until && user.locked_until > now2) {
...
async function checkPasswordExpiry(db, userId) {
  ...
  const now2 = Math.floor(Date.now() / 1e3);
  if (user.password_expires_at && user.password_expires_at < now2) {
    return { expired: true, expiresAt: user.password_expires_at };
  }
  return { expired: false, expiresAt: user.password_expires_at };
}
...
async function checkSessionTimeout(db, refreshToken) {
  ...
  const now2 = Math.floor(Date.now() / 1e3);
  if (session.expires_at < now2) {
    await invalidateSession(db, session.id);
    return { valid: false, reason: "Session expired" };
  }
  const inactivityDuration = now2 - session.last_activity;
  ...
  const sessionAge = now2 - session.created_at;
...
async function updateSessionActivity(db, sessionId, ipAddress, userAgent) {
  ...
  await db.prepare(`
    ...
    WHERE id = ?
  `).bind(now2, ipAddress, userAgent, sessionId).run();
}
...
async function createSession(db, userId, refreshToken, ipAddress, userAgent) {
  const now2 = Math.floor(Date.now() / 1e3);
  const expiresAt = now2 + SESSION_ABSOLUTE_TIMEOUT_SECONDS;
  const sessionId = crypto.randomUUID();
  await db.prepare(`
    INSERT INTO sessions (id, user_id, refresh_token, expires_at, last_activity, ip_address, user_agent, created_at)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
  `).bind(sessionId, userId, refreshToken, expiresAt, now2, ipAddress, userAgent, now2).run();
  return {
    id: sessionId,
    userId,
    lastActivity: now2,
...
async function cleanupExpiredSessions(db) {
  ...
  `).bind(now2, now2 - MAX_INACTIVITY_SECONDS).run();
...
async function setupMFA(db, userId) {
  ...
  await db.prepare(`
    ...
    VALUES (?, ?, ?, 0, ?)
  `).bind(userId, secret, JSON.stringify(backupCodes), now2).run();
  ...
  try {
    ...
    await c.env.DB.prepare(
      `INSERT INTO users (id, email, password_hash, name, role, user_type, tenant_id, password_changed_at, password_expires_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`
    ).bind(userId, email, passwordHash, name || "", "user", userType, tenantId, now2, passwordExpiresAt).run();
...
async function rateLimitCheck(env, key, limit, windowMs) {
  try {
    const now2 = Date.now();
    const windowKey = `ratelimit:${key}:${Math.floor(now2 / windowMs)}`;
  ...
  try {
    ...
    await withRetry(async () => {
      await c.env.DB.prepare(`
        ...
        VALUES (?, 1, ?, ?, ?)
      `).bind(fileId, checksum, userId, now2).run();
    }, 3);
    await auditLogger(c.env, {
      tenant_id: tenantId,
      user_id: userId,
...
router.post("/", async (c) => {
  ...
  try {
    ...
    const assessment = {
      ...
      completed_at: null,
      created_by: userId,
      created_at: new Date(now2 * 1e3).toISOString(),
      updated_at: new Date(now2 * 1e3).toISOString()
    };
    return c.json(assessment);
  } catch (error) {
...
// .wrangler/tmp/bundle-HvQAwE/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-HvQAwE/middleware-loader.entry.ts
